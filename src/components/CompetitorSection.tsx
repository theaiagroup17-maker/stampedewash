...
import { BRANDS, BRAND_KEYS, getBrand } from '@/lib/brands';
...
const EDIT_CITIES = ['Calgary', 'Airdrie', 'Cochrane', 'Okotoks'] as const;
const WASH_TYPES = ['All', 'tunnel', 'automatic', 'express', 'full_service'] as const;
const EDIT_WASH_TYPES = ['tunnel', 'automatic', 'express', 'full_service', 'detail'] as const;

async function updateCompetitor(id: string, field: string, value: string) {
...
export default function CompetitorSection({
  ...
  const handleFieldChange = async (compId: string, field: string, value: string) => {
    try {
      await updateCompetitor(compId, field, value);
      ...
      {expanded && (
        <div className="px-4 pb-4 space-y-3">
          ...
          <div className="flex gap-2 items-center">
            ...
            <button onClick={() => setShowList(!showList)} className="text-[10px] text-stampede-red font-medium hover:underline">
          ...
          {showList && (
            <div className="border-t border-gray-100 pt-3 space-y-2 max-h-64 overflow-y-auto">
              <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">All Competitors (editable)</p>
              {filteredCompetitors.map(comp => {
                const brand = getBrand(comp.brand);
                return (
                  <div key={comp.id} className="bg-gray-50 rounded-lg p-2 space-y-1.5">
                    <div className="flex items-center gap-2">
                      <span className="w-3 h-3 rounded-full flex-shrink-0" style={{ background: brand.color }} />
                      <span className="text-xs font-semibold text-gray-800 truncate flex-1">{comp.name}</span>
                    </div>
                    <p className="text-[10px] text-gray-500 truncate">{comp.address}</p>
                    <div className="flex gap-1.5 flex-wrap">
                      {/* Brand selector */}
                      <select value={comp.brand} onChange={(e) => handleFieldChange(comp.id, 'brand', e.target.value)}
                        className="text-[10px] px-1.5 py-0.5 border border-gray-200 rounded bg-white focus:outline-none focus:ring-1 focus:ring-stampede-red/30">
                        {BRAND_KEYS.map(k => (
                          <option key={k} value={k}>{BRANDS[k].acronym} — {BRANDS[k].name}</option>
                        ))}
                      </select>
                      {/* Wash type selector */}
                      <select value={comp.wash_type || ''} onChange={(e) => handleFieldChange(comp.id, 'wash_type', e.target.value)}
                        className="text-[10px] px-1.5 py-0.5 border border-gray-200 rounded bg-white focus:outline-none focus:ring-1 focus:ring-stampede-red/30">
                        {EDIT_WASH_TYPES.map(wt => (
                          <option key={wt} value={wt}>{wt.replace('_', ' ')}</option>
                        ))}
                      </select>
                      {/* City selector */}
                      <select value={comp.city || ''} onChange={(e) => handleFieldChange(comp.id, 'city', e.target.value)}
                        className="text-[10px] px-1.5 py-0.5 border border-gray-200 rounded bg-white focus:outline-none focus:ring-1 focus:ring-stampede-red/30">
                        {EDIT_CITIES.map(c => (
