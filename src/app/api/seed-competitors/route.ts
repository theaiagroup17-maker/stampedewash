...
export async function POST(_req: NextRequest) {
  try {
    ...
    for (const comp of SEED_COMPETITORS) {
      ...
      // Try to geocode for more precise coordinates if we have Google Maps API key
      ...
      // Check if exists by name + address
    ...
    console.log(`[seed-competitors] Inserted: ${inserted}, Updated: ${updated}, Total: ${total}`);
    return NextResponse.json({ count: total, inserted, updated });
  } catch (err: any) {
